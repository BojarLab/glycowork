...
def test_convert_cfg_to_iupac(parser, input_glycan: str, expected_glycan: str):
    ouput = parser.parse(glycan=input_glycan)
    # Test to make sure it can be parsed by glycowork
    glycan_to_nxGraph(glycan=ouput)
    assert ouput == expected_glycan
